...
class Tabel extends JFrame {  //Creare Tabeluri cu zboruri
	...
	ArrayList <Zbor> zbor = nrLocuri.getListaZboruri();
	...
	ArrayList<Zbor> xx=A.getListaZboruri();
	...
	DefaultTableModel model = (DefaultTableModel) table.getModel();
	InformatiiZboruri A=new InformatiiZboruri();
	ArrayList<Zbor> cautareZbor=A.getListaZboruri();
	...
	if(gasit==true)
	{
		String numeFisier="src\\EBileteAvion\\Rezervari.txt";
		try 
		(BufferedWriter scriere=new BufferedWriter(new FileWriter(numeFisier))){  // scriere rezervarii in fisier
		scriere.write((String)data[1]+"-"+(String)data[2]+"-"+(String)data[3]+"-"+(String)data[4]);
		}
		catch(IOException e) {
			 e.printStackTrace();
		}
